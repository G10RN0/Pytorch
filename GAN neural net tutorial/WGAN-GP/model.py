...
class Critic(nn.Module):
    def __init__(self, input, hidden):
        ...
        self.instance_norm_2d_1 = nn.InstanceNorm2d(hidden*2, affine=True)
        self.conv_2 = nn.Conv2d(hidden*2, hidden*4, kernel_size=4,stride=2,padding=1)#8x8
        self.instance_norm_2d_2 = nn.InstanceNorm2d(hidden*4, affine=True)
        self.conv_3 = nn.Conv2d(hidden*4, hidden*8, kernel_size=4,stride=2,padding=1)#4x4
        self.instance_norm_2d_3 = nn.InstanceNorm2d(hidden*8, affine=True)
    ...
    def forward(self, x):
        ...
        x = self.L_relu(self.instance_norm_2d_1(self.conv_1(x)))
        x = self.L_relu(self.instance_norm_2d_2(self.conv_2(x)))
        x = self.L_relu(self.instance_norm_2d_3(self.conv_3(x)))
